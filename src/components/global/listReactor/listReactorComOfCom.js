export default {
  ...
  methods: {
    ...
    getPointReactions () {
      ...
      axios.post(this.$store.state.baseUrl + 'pointReactionsComOfCom.php', {
        comOfCom: this.id
      })
        .then((response) => {
    ...
    getReactor (reaction) {
      ...
      const axios = require('axios')
      axios.post(this.$store.state.baseUrl + 'listComOfComReacteur.php', {
        comOfCom: this.id,
  ...
  mounted () {
    this.$root.$on('lsReactComOfClick', data => {
      this.id = data
